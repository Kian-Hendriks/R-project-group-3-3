...
# Het CBS-bestand is 'breed' en heeft 6 koprijen: titel, 2 lege regels,
# 'Onderwerp', de provincienamen (rij 5) en de eenheid (rij 6). De data
# beginnen pas op rij 7. Daarom lezen we zonder header en bouwen we zelf
# een lang bestand met de kolommen Perioden / Regio.s / dichtheid.
ruw <- read.csv(
...
head(bevolking_breed)   # View() werkt niet tijdens het knitten
```
...
ggplot(nl_map) +
  ...
  scale_fill_distiller(
    ...
    name = "Average spendable income (x100 €)"
  ) +
  theme_void() +
  labs(
...
ggplot(nl_map) +
  ...
  theme_void() +
  labs(
    title = "Average Houseprice per Province",
    subtitle = "Average between 2015-2023"
  )
```
...
ggplot(data = df_plot, aes(x = dichtheid, y = huizenprijs, label = Provincies)) +
  ...
  labs(
    title = "Correlatie Bevolkingsdichtheid vs. Huizenprijzen (2020-2022)",
    subtitle = "Met lineaire regressielijn",
    x = "Bevolkingsdichtheid (inwoners per km²)",
    y = "Gemiddelde huizenprijs"
  ) +
  theme_minimal()
```
